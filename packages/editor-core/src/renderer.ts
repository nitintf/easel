...
export class SkiaRenderer {
  ...
  render(
    ...
    this.drawVectorEditOverlay(canvas, graph, overlays.vectorEditState)
  ...
  private drawVectorEditOverlay(
    canvas: Canvas,
    graph: SceneGraph,
    state: RenderOverlays['vectorEditState'],
  ): void {
    if (!state) return
    const node = graph.getNode(state.nodeId)
    if (!node?.vectorNetwork) return

    const abs = graph.getAbsolutePosition(state.nodeId)
    ...
    const toScreen = (x: number, y: number) => ({
      x: (abs.x + x) * this.zoom + this.panX,
      y: (abs.y + y) * this.zoom + this.panY,
    })
    ...
    // Draw vertices (selected vertex gets a larger highlight)
    for (let i = 0; i < vertices.length; i++) {
      const v = toScreen(vertices[i].x, vertices[i].y)
      const isSelected = i === state.selectedVertex
      const radius = isSelected ? PEN_VERTEX_RADIUS + 2 : PEN_VERTEX_RADIUS
      if (isSelected) {
        // Draw a selection highlight ring
        this.auxStroke.setColor(this.ck.Color4f(0.30, 0.52, 0.96, 1))
        this.auxStroke.setStrokeWidth(2)
        this.auxStroke.setPathEffect(null)
        canvas.drawCircle(v.x, v.y, radius + 2, this.auxStroke)
      }
      canvas.drawCircle(v.x, v.y, radius, vertexFill)
      canvas.drawCircle(v.x, v.y, radius, vertexStroke)
    }
