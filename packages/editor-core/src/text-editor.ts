...
export class TextEditor {
  ...
  stop(): { nodeId: string; text: string; originalText: string } | null {
    if (!this._state) return null
    const result = { nodeId: this._state.nodeId, text: this._state.text, originalText: this._originalText }
  ...
  rebuildParagraph(node: SceneNode): void {
    ...
    s.paragraph?.delete()
    // Use editor's current text for paragraph, not the (potentially stale) node.text
    const nodeWithText = { ...node, text: s.text }
    s.paragraph = this.renderer.buildParagraph(nodeWithText as SceneNode)
  }
